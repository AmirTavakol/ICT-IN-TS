...
def CpH_filterBookings(city):    
    pipeline = [
        ...
        # Filter Block
        {
            '$match': {"$and": [{'moved': True}, {'duration': {'$gte': 180, '$lte': 10800}}]}
        },
        {
            '$group': { '_id': '$hourOfDay', 
                'count':{'$sum':1}
...
def CpH_filterParkings(city):    
    pipeline = [
        ...
        # Filter Block
        {
            '$match': {"$and": [{'moved': False}, {'duration': {'$gte': 180, '$lte': 86000}}]}
        },
...
books_pipe = CpH_filterBookings(city)
parks_pipe = CpH_filterParkings(city)
